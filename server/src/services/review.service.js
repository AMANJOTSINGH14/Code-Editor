 ...
 * When GEMINI_MOCK=true, returns a fake model for E2E / dev testing.
 * @returns {Promise<Object>} Gemini chat model.
 */
async function getChatModel() {
  // ── Mock mode (E2E tests / local dev without a real API key) ──
  if (process.env.GEMINI_MOCK === "true") {
    const mockText =
      ...
      "_This review was generated by the mock Gemini adapter (GEMINI_MOCK=true)._";

    return {
      invoke: async () => ({ content: mockText }),
      stream: async function* () {
        for (const word of mockText.split(" ")) {
          yield { content: word + " " };
          await new Promise((r) => setTimeout(r, 60)); // slow enough for Stop-button test
        }
      }
    };
  }
